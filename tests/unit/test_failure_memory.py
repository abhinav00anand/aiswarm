...
class TestFailureMemory:
    ...
    def test_record_persists_to_disk(self, isolated_store: Path) -> None:
        mem = FailureMemory()
        mem.record(FailureRecord(
            task_id="t1", task_title="Fix bug", error_pattern="circular import",
            error_source="compiler", resolution="moved import inline",
        ))
    ...
    def test_find_similar_matches_substring(self, isolated_store: Path) -> None:
        mem = FailureMemory()
        mem.record(FailureRecord(
            task_id="t1", task_title="T", error_pattern="numpy not available",
            error_source="compiler", resolution="add numpy to requirements",
        ))
    ...
    def test_find_similar_case_insensitive(self, isolated_store: Path) -> None:
        mem = FailureMemory()
        mem.record(FailureRecord(
            task_id="t1", task_title="T", error_pattern="Circular Import",
            error_source="compiler", resolution="fix",
        ))
    ...
    def test_find_similar_respects_top_k(self, isolated_store: Path) -> None:
        ...
        for i in range(5):
            mem.record(FailureRecord(
                task_id=f"t{i}", task_title="T", error_pattern="timeout",
                error_source="test", resolution=f"fix-{i}",
            ))
    ...
    def test_resolution_hint_includes_resolution_text(self, isolated_store: Path) -> None:
        mem = FailureMemory()
        mem.record(FailureRecord(
            task_id="t1", task_title="T", error_pattern="hardcoded secret",
            error_source="security_critic", resolution="use env var instead",
        ))
    ...
    def test_most_recent_match_returned_first(self, isolated_store: Path) -> None:
        mem = FailureMemory()
        mem.record(FailureRecord(
            task_id="t1", task_title="T", error_pattern="deadlock",
            error_source="orchestrator", resolution="old-fix", resolved_at=100.0,
        ))
        mem.record(FailureRecord(
            task_id="t2", task_title="T", error_pattern="deadlock",
            error_source="orchestrator", resolution="new-fix", resolved_at=200.0,
        ))
