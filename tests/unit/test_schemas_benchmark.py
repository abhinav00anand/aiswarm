...
class TestBenchmarkSuite:
    def test_default_tolerance_is_10_percent(self) -> None:
        s = BenchmarkSuite(suite_id="s1", task_class="PERFORMANCE", description="d", metrics=["latency"])
    ...
    def test_can_set_custom_tolerance(self) -> None:
        s = BenchmarkSuite(
            suite_id="s1", task_class="PERFORMANCE", description="d",
            metrics=["throughput"], tolerance_pct=5.0,
        )
...
class TestBenchmarkRun:
    ...
    def test_metrics_dict_can_carry_values(self) -> None:
        r = BenchmarkRun(
            run_id="r1", suite_id="s1", task_id="t1",
