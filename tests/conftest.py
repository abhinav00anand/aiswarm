...
def approved_task(sample_task, tmp_path):
    ...
    from aiswarm.schemas.task import (
        TaskState, CriticReview, ReviewDecision, CompilerOutput, TestOutput,
    )
    ...
    sample_task.reviews = [
        CriticReview(critic_role="architecture", decision=ReviewDecision.APPROVE, production_ready=True, score=85),
        CriticReview(critic_role="performance", decision=ReviewDecision.APPROVE, production_ready=True, score=85),
        CriticReview(critic_role="security", decision=ReviewDecision.APPROVE, production_ready=True, score=90),
    ]
