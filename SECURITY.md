# Zymis Security Policy & Threat Model

---

## 🔒 Security Architecture & Vision

**Zymis** is designed with a **Zero-Trust Security Architecture**. Because autonomous AI agents generate, compile, and execute code dynamically, Zymis enforces strict boundaries to ensure generated code cannot breach process isolation, leak credentials, access host file systems, or exceed token budgets.
...
| **Observability** | `AuditLedger` | Thread-safe, append-only JSONL log (`~/.zymis/audit.jsonl`) with startup recovery. |
...
`APIKeyValidator` inspects the environment during initialization (`zymis/security/auth.py`). If no valid provider key (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`) is found, initialization halts immediately with a `SecurityAuthError`.

### 2. Subprocess Execution Sandbox (`ExecutionSandbox`)
All code execution occurs inside isolated subprocesses managed by `ExecutionSandbox` (`zymis/security/sandbox.py`):
...
`SecretRedactor` (`zymis/security/redaction.py`) intercepts all strings flowing to logs, stdout, prompts, or audit event payloads:
```text
...
Every event is assigned a UUID, timestamped, typed (`ROUTE_DECISION`, `TOOL_SPAWN`, `POLICY_VIOLATION`, `MERGE`), and appended to `~/.zymis/audit.jsonl` using an async `Lock` (`zymis/security/audit.py`).

---

## 🚨 Confidential Vulnerability Reporting

If you discover a security vulnerability in Zymis:
