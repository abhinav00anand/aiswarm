...
class CapabilityRequest(BaseModel):
    ...
    capability_name: str = Field(
        ..., description="Name of the requested capability (e.g. pytest, tiny_coder, ruff)"
    )
    requester_role: str = Field(
        ..., description="Role requesting the capability (e.g. host2, boss)"
    )
    parameters: dict[str, Any] = Field(
        default_factory=dict, description="Parameters for the capability invocation"
    )
...
class EscalationPacket(BaseModel):
    ...
    task_id: str = Field(...)
    reason: str = Field(
        ..., description="Why the fast lane escalated (e.g. complexity, retries, security risk)"
    )
    completed_steps: list[str] = Field(
        default_factory=list, description="Steps completed before escalation"
    )
    failed_stage: str | None = Field(default=None, description="Stage where failure occurred")
    artifacts_created: list[str] = Field(
        default_factory=list, description="Paths to generated scratch artifacts"
    )
    remaining_risks: list[str] = Field(
        default_factory=list, description="Identified unmitigated risks"
    )
    suggested_next_action: str = Field(
        default="DECOMPOSE_AND_PLAN", description="Suggested action for Boss"
    )
