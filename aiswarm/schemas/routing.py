...
class RouteDecision(BaseModel):
    ...
    route: ExecutionMode = Field(
        ..., description="Target execution lane (FAST, PRODUCTION, or HYBRID)"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Router confidence score between 0.0 and 1.0"
    )
    ...
    estimated_cost_usd: float = Field(
        default=0.01, ge=0.0, description="Estimated execution cost in USD"
    )
    estimated_runtime_seconds: float = Field(
        default=30.0, ge=0.0, description="Estimated wall-clock execution time"
    )
    required_capabilities: list[str] = Field(
        default_factory=list, description="Capabilities required for execution"
    )
